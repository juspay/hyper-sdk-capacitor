...
export class HyperServicesWeb extends WebPlugin implements HyperServicesPlugin {
  ...
  async createHyperServices(
    options?: { clientId?: string; service?: string },
  ): Promise<void> {
    return this.createHyperServicesInternal(options?.clientId, undefined, options?.service);
  }

  async createHyperServicesWithTenantId(
    options: { tenantId: string; clientId?: string; service?: string; env?: string },
  ): Promise<void> {
    return this.createHyperServicesInternal(options.clientId, options.tenantId, options.service, options.env);
  }
  ...
  private getSDKBaseDomain(tenantId: string = "", env?: string) {
    switch (tenantId) {
      case 'juspayglobal':
      case 'jp_global':
        return env == "sandbox" ? "https://payments.sandbox.juspay.io/payment-page": "https://payments.juspay.io/payment-page"
      default:
  ...
  async terminate(): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.hyperServices) {
        reject(
          new Error(
            'HyperServices instance not created or already terminated!',
          ),
        );
        return;
      }
